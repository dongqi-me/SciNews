...
class LEDEncoderSelfAttention(nn.Module):
    ...
    def forward(
        ...
        *attention_window* happens in [`LEDEncoderModel.forward`] to avoid redoing the padding on each layer.
        ...
        # query_vectors.shape = [batch_size, seq_len, num_heads, head_dim]

        attn_scores = self._sliding_chunks_query_key_matmul(
            query_vectors, key_vectors, self.one_sided_attn_window_size
        )

        #attn_scores.shape = [batch_size,padding,attention_head,2*window_overleap+1=1025]
...
class LEDEncoderAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.longformer_self_attn(
            ...
            RST_Atten_Doc_Paded=RST_Atten_Doc_Paded
        )
...
class LEDEncoder(LEDPreTrainedModel):
    ...
    def forward(
        ...
        for chunk in Raw_sentence[0].split('. '):
            if chunks and len(chunk) + len(chunks[-1]) < threshold:
                chunks[-1] += ' ' + chunk + '.'
            else:
                chunks.append(chunk + '.')

        _, EDU_Breaks, Parsing_Tree, Relation_Values = inference(ParsingNetModel, bert_tokenizer, chunks, batch_size=1)
        RST_Atten_Doc = pd.DataFrame()
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    def create_custom_forward(module):
                        def custom_forward(*inputs):
                            return module(*inputs, is_global_attn, output_attentions,RST_Atten_Doc_Paded)
